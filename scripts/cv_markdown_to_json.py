...
def parse_skills(skills_dir):
    """Parse skills from the _skills directory."""
    skills = []
    
    if not os.path.exists(skills_dir):
        return skills
    
    for skills_file in sorted(glob.glob(os.path.join(skills_dir, "*.md"))):
...
def create_cv_json(md_file, config_file, repo_root, output_file):
    ...
    # Add skills
    cv_json["skills"] = parse_skills(os.path.join(repo_root, "_skills"))
